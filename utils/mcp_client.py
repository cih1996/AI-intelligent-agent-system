...
class MCPClientManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path:
            ...
            self.config_path = project_root / 'mcp.json'
        ...
        for server_name, client in self.clients.items():
            try:
                ...
                if 'error' in init_result:
                    print(f"  ✗ {server_name}: 初始化失败 - {init_result['error'].get('message', '未知错误')}")
                    continue
                ...
                if 'result' in tools_result:
                    ...
                    print(f"  ✓ {server_name}: 连接成功，已注册 {len(tools)} 个工具")
                else:
                    print(f"  ⚠ {server_name}: 连接成功，但无法获取工具列表")
                    
            except Exception as e:
                print(f"  ✗ {server_name}: 连接失败 - {str(e)}")
        
        print(f"[MCP Client Manager] 总共注册 {len(self.tool_to_server)} 个工具")
